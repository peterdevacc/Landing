...
class HelpDAOTest {
    ...
    @Test
    fun helpTest() = runBlocking {
        val id: Long = 1
        val expectHelpList = getExpectHelpList().filter { it.catalogId == id }
        val helpList = dao.getHelpListByCatalogId(id)
        assertEquals(expectHelpList, helpList)
    }
    ...
    private fun getExpectHelpList() : List<Help> =
        Gson().fromJson(jsonStr, object : TypeToken<List<Help>>() {}.type)
